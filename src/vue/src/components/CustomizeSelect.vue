<template>
  <div class="row">
    ...
    <div class="col-lg-6 mt-3 mt-lg-0" v-show="showAdvancedOptions">
      <label :for="idPrefix + '-custom'" class="form-label"
        >{{ customLabel }} internal name (<a href="#" role="button" @click.prevent="customize"
          >customize</a
        >)</label
      >
      <input
  ...
  props: {
    ...
    idPrefix: {
      ...
      default(rawProps: { label: string }) {
        const idPrefix = rawProps.label.replaceAll(" ", "-").toLowerCase()
        return idPrefix
      },
    },
  },
  ...
  watch: {
    showAdvancedOptions: function (newVal: boolean) {
      if (!newVal && this.selectValue == "custom") {
        this.selectValue = this.allowedValues.includes(this.defaultValue)
          ? this.defaultValue
